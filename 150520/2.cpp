...
public :
	...
	T& operator*() { return *pObj; } // Car 리턴 시, 임시 객체가 생성되므로 참조를 리턴한다. 여기서는 RVO 가 아닌 참조를 리턴
};

// 스마트 포인터는 진짜 포인터가 아닌 객체이므로 사용자가 객체의
// 생성/복사/대입/소멸의 과정을 제어할 수 있다.
// 대표적인 활용 예) 소멸자를 사용한 자동삭제

void main()
{
	{
		Sptr<Car> pCar = new Car;
		pCar->go();	// pCar.operator->()go; (x) 함수 호출 () 이 끝나면 ; 와야되나, 토큰이 붙었다.
					// 결국 이렇게 컴파일러가 바꿈, (pCar.operator->())->go(); 

		(*pCar).go();	// 역참조 연산자도 가능해야함
						// pCar.operator*().go()
...
//void main()
//{
//	// 힙 객체의 소멸자가 호출되는 시점은 delete 연산자를 사용할 때
