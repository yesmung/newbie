...
class Trainer(BaseTrain):
            ...
            if epoch == 0:
                ...
                if epoch % validation_steps == 0:
                    if currentLoss > np.double(g_loss[0]):
                        currentLoss = np.double(g_loss[0])
                        mlflow.keras.log_model(self.model.generator, artifact_path="models_best_generator")
    ...
    def sample_images(self, epoch, imgs_lr, imgs_hr):
        imgsavepath = self.config.META_DB.path + self.config.META_DB.name + '/images'
        os.makedirs(imgsavepath + '/%s' % self.dataset_name, exist_ok=True)
        ...
        fig.savefig(imgsavepath + "/%s/%08d.png" % (self.dataset_name, epoch))
        plt.close()

        mlflow.ActiveRun(self.config.mlrun)
        mlflow.log_artifact(imgsavepath + "/%s/%08d.png" % (self.dataset_name, epoch), "validation_images")
