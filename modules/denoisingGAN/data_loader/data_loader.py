...
class DataLoader(BaseDataLoader):
	...
	def get_data_online(self, batch_size=8, descale=2, from_config=False):
		...
		print("... split list")
		img_input_s = split_list(img_input, [train_valid_ratio, 1-train_valid_ratio], shuffle=True, data_length=[epoch_size*step_size*batch_size, epoch_size*batch_size])
		img_target_s = split_list(img_target, [train_valid_ratio, 1-train_valid_ratio], shuffle=True, data_length=[epoch_size*step_size*batch_size, epoch_size*batch_size])
			...
			def get_data_from_addr(imgaddr1, imgaddr2):
				...
				with denv.begin(write=False) as txn:
					...
					if np.random.randint(0, high=5, size=1)[0]==1:
						...
						img1.save(bufflow, 'JPEG', quality=int(np.random.randint(1, high20, size=1)[0]))
