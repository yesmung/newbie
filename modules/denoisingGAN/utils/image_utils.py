...
def compute_maps(heatmap, image_height, image_width, lines, heatmap_affinity=None, descale=2, make_channel=False):
	...
	src = np.array([[0, 0], [heatmap.shape[1],0], [heatmap.shape[1],heatmap.shape[0]], [0, heatmap.shape[0]]]).astype('float32')
	...
	if make_channel == False:
		...
		return mp.concatenate([textmap[..., np.newaxis], linkmap[..., np.newaxis], emptymap[..., np.newaxis]], axis=2).clip(0, 255)
...
def getBoxes(y_pred, detection_threshold=0.7, text_threshold=0.4, link_threshold=0.4, size_threshold=10, dscale=2):
	...
	for y_pred_cur in y_pred:
		...
		for component_id in range(1, n_components):
			...
			x, y, w, h =[
				stats[component_id, key] for key in [cv2.CC_STAT_LEFT, cv2.CC_STAT_TOP, cv2.CC_STAT_WIDTH, cv2.CC_STAT_HEIGHT]
			]
