...
class DataLoader(BaseDataLoader):
	...
	def get_data_online(self, batch_size=8, size=512, distanceRatio=[1.5, 1], sratio=[5,1,1,5], amp=[1,1], descale=2, make_channel=False, from_config=False, use_otsu=False):
		...
		# create heatmap
		heatmap = image_utils.get_asymmetric_gaussian_heatmap(size, distanceRatio[0], sx=sratio[0], sy=sratio[1], amp=amp[0])
		heatmap_affinity = image_utils.get_asymmetric_gaussian_heatmap(size, distanceRatio[1], sx=sratio[2], sy=sratio[3], amp=amp[1])
		...
		img_list = read_all_keys_from_meta_db(db_meta, 'img-', remove_list=True)
		char_list = read_all_keys_from_meta_db(db_meta, 'char_c', remove_list=True)
		...
		img_s = split_list(imgs, [train_valid_ratio, 1-train_valid_ratio], shuffle=True, data_length=[epoch_size*step_size*batch_size, epoch_size*batch_size])
		coord_s = split_list(chars, [train_valid_ratio, 1-train_valid_ratio], shuffle=True, data_length=[epoch_size*step_size*batch_size, epoch_size*batch_size])
		...
		def make_tfdataset(imgs, coods):
			...
			def get_data_from_addr(imgaddr, lineaddr):
				...
				with denv.begin(write=False) as txn:
					...
					img = np.array(img)
					tlines = decode_pd(db_cs.get(str(lkey).encode()), sep=get_value_by_key(denv, 'sep'))
	...
	def get_inference_data_online(self, use_maindb=True):
		...
		def make_tfdataset(imgs):
			...
			return tf.data.Dataset.from_tensor_slices(imgs).map(preprocess_raw, num_parallel_calls=tf.data.experimental.AUTOTUNE)
